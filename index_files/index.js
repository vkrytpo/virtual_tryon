...
function(module, exports, __webpack_require__) {
	...
	function updateElementTransform() {
		  var s =1;
	        ...
	        'scale(' + transform.scale*s + ', ' + transform.scale*s + ')',
	        'rotate3d('+ transform.rx*s +','+ transform.ry*s +','+ transform.rz +','+  String(Number(transform.angle)*s) + 'deg)'
	    ];
			value = value.join(" ");
	...
	function onRotate(ev) {
	    if(ev.type == 'rotatestart') {
	        initAngle = transform.angle || 0;
					startAngle = ev.rotation;
		  }
		  el.className = '';
	    transform.rz = 1;
    	transform.angle = initAngle + ev.rotation - startAngle;
	    logEvent(ev);
	    requestElementUpdate();
	}
